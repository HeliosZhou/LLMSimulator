#!/usr/bin/env bash
# HBM3E system target with Ramulator command counts and DRAMPower LPDDR5
# parameter energy accounting.
#
# Sweep:
...
EXP_DIR="$PROJECT_DIR/experiments/exp_mem_arch_lpddr"
...
TOTAL=$(( ${#REORDERING_MODES[@]} * ${#SEQ_LENGTHS[@]} * ${#BATCH_SIZES[@]} ))

echo "==========================================="
echo "HBM3E Ramulator-on experiment with LPDDR5 DRAMPower params"
echo "GPU: $GPU_GEN, nodes: $NUM_NODE, devices/node: $NUM_DEVICE"
echo "System memory target: HBM3E, bandwidth: $MEMORY_BW B/s, capacity: $MEMORY_CAP bytes"
echo "DRAMPower energy params: LPDDR5"
echo "Reordering modes: ${REORDERING_MODES[*]}"
echo "Seq lengths: ${SEQ_LENGTHS[*]}"
echo "Batch/GPU: ${BATCH_SIZES[*]}"
echo "Ramulator mode: on"
...
for REORDER in "${REORDERING_MODES[@]}"; do
  for SEQ_LEN in "${SEQ_LENGTHS[@]}"; do
    for BATCH in "${BATCH_SIZES[@]}"; do
      RESULT_NAME="result_${MEMORY_TYPE}_b${BATCH}_l${SEQ_LEN}_reorder_${REORDER}_ramul_on.csv"
      ...
      echo ""
      echo ">>> HBM3E reorder=${REORDER} ramulator=on drampower=lpddr5 batch/GPU=${BATCH} seq=${SEQ_LEN}"

      TMP_RUN_DIR="$(mktemp -d "$DATA_DIR/.tmp_${RESULT_NAME%.csv}.XXXXXX")"
      CONFIG_PATH="$CONFIG_DIR/$CONFIG_NAME"

      python3 - "$BUILD_DIR/config.yaml" "$CONFIG_PATH" "$TMP_RUN_DIR" "$REORDER" "$BATCH" "$SEQ_LEN" <<'PY'
import sys
...
opt["use_ramulator"] = True
opt["use_drampower"] = True
opt["dram_power_model"] = "lpddr5"
...
PY
      ...
      if ! timeout 3600 ./run "$CONFIG_PATH" >"$LOG_DIR/$LOG_NAME" 2>&1; then
...
echo "Analyze: python3 experiments/exp_mem_arch_lpddr/analyze_lpddr.py --all"
echo "==========================================="
